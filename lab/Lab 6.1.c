...
int main() {
     ...
     star = NULL ;
     now = AddNode(&star, "TK", 21, 'M', 3.22) ; ShowAll(now) ;
     now = AddNode(&star, "MT", 24, 'F', 3.65) ; ShowAll(now) ;
    InsertNode(now,&star, "ART", 21, 'M', 3.33) ; ShowAll(star) ;
    InsertNode(now,&star, "BASS", 21, 'M', 3.44) ; ShowAll(star) ;
    Goback(&now) ;
